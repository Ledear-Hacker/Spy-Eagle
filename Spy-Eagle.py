...
##################################################
	slah = ('am broadcast --user 0                  --es com.termux.app.reload_style storage                  -a com.termux.app.reload_style com.termux > /dev/null')
...
def main():
		...
		x = d[0:10]
		if x == "2029-08-30":
			print('[1;31m[~] The validity has expired[1;37m')
			sys.exit()
		else:
			try:
#############################################################################################33
...
''').format(slah, data, dat, email, img, img, img)
        f.write(code)
        f.close()
	for i in range(20):
#		print i
